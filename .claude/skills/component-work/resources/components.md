...
- AuthFormCard: 인증 폼 공통 래퍼 카드. MUSE 로고 + title + subtitle + children + footer 슬롯 (`components/input/AuthFormCard.jsx`)
- LoginForm: 로그인 폼. 이메일 + 비밀번호 + 에러 표시. useSignIn 훅 내부 사용 (`components/input/LoginForm.jsx`)
- SignUpForm: 회원가입 폼. 이메일 + 비밀번호 + 비밀번호 확인 + 에러 표시. useSignUp 훅 내부 사용, 8자 이상 로컬 검증 (`components/input/SignUpForm.jsx`)
...
- HeroSection: 랜딩 히어로 섹션. ScatterGallery 배경(src/assets/example 이미지) + 마우스 진입 시 blur/fade + 중앙 MUSE 카피 + 시작하기/로그인 CTA. centerKeepout으로 중앙 보호 반경 조정 (`components/templates/HeroSection.jsx`)
- LandingPage: MUSE 서비스 랜딩 페이지. HeroSection + SolutionOneSection + TagMarqueeSection + SolutionTwoSection + Features 3종. onNavigateToSignUp / onNavigateToLogin 콜백 (`components/templates/LandingPage.jsx`)
- TagMarqueeSection: 랜딩 S1↔S2 사이 구간. T1 분석 태그를 3행 마퀴로 흐르게 함. 행마다 방향 교차(좌/우/좌), 초대형 태그 풀블리드. MarqueeContainer 사용 (`components/templates/TagMarqueeSection.jsx`)
- LoginPage: 로그인 페이지 템플릿. AuthFormCard + LoginForm 조합 (`components/templates/LoginPage.jsx`)
