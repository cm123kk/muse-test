...
${extractedPool.some((r) => r.useLayers.length > 0)
  ...
  // Start progress state. color + visualDirection run in parallel (VD is a separate call).
  onProgress?.(
    PROGRESS_KEYS_SYSTEM.map((key) => ({
      key,
      status: (key === 'color' || key === 'visualDirection') ? 'running' : 'pending',
    })),
  );

  const MAX_TOKENS = 8192;
  const VD_MAX_TOKENS = 2048;
  const [coreSchema, vdSchema, designmdSchema] = TASK_ANALYZE_TOKENS.toolSchemas;
  ...
  const phase1Content = [
    ...content,
    {
      type: 'text',
      text: `=== PHASE 1 (tokens) ===
This call emits ONLY tokens.{color, typography, layout, gradient} via ${coreSchema.name}.
DO NOT include visualDirection here (a separate submit_visual_direction call handles it).
...
Emit ONLY the visualDirection (markdown + tags) via ${vdSchema.name}.
Do NOT emit any tokens here.
visualDirection.markdown must be 200+ chars filling sections 1-6 of the template (Project Overview, Overall Direction, Visual Direction Tags, Tone & Mood, Implementation Guidelines, Elements to Avoid).`,
    },
  ];

  const callVisualDirection = async (extraInstruction = '') => {
    const messagesContent = extraInstruction
      ? [...vdContent, { type: 'text', text: extraInstruction }]
      : vdContent;
    const res = await callAnthropic({
      model: baseModel,
      max_tokens: VD_MAX_TOKENS,
      system: TASK_ANALYZE_TOKENS.systemPrompt,
      tools: [vdSchema],
      tool_choice: { type: 'tool', name: vdSchema.name },
      messages: [{ role: 'user', content: messagesContent }],
    });
    return { res, input: extractToolInput(res, vdSchema.name) };
  };
  ...
  const checkCoreEmpties = (input) => {
    const t = input?.tokens || {};
    return ['color', 'typography', 'layout', 'gradient']
      .filter((k) => !(Array.isArray(t[k]) && t[k].length > 0));
  };
  /** VD is generated by a separate parallel call. Empty = needs a retry. */
  const checkVdEmpty = (vd) => !vd?.markdown || vd.markdown.length < 200;
  ...
  // Phase 1 tokens and the visual direction do not depend on each other, so run them in
  // parallel. Wall-clock time becomes max(tokens, VD) instead of the sum, and VD can no
  // longer be truncated by a large token payload (it has its own call + budget).
  const [p1r, vdr] = await Promise.all([callPhase1(), callVisualDirection()]);
  let { res: p1Res, input: phase1 } = p1r;
  let vd = vdr.input;
  ...
  // eslint-disable-next-line no-console
  console.log('[runAnalyzeTokens] Phase 1 result:', summarizePhase1(phase1), { vdMarkdownLen: vd?.markdown?.length || 0 });

  // Visual direction defense: if it came back empty/short, regenerate it once (explicit, not a silent fallback).
  if (checkVdEmpty(vd)) {
    // eslint-disable-next-line no-console
    console.warn('[runAnalyzeTokens] visualDirection empty/short -> retrying once');
    const vdRetry = await callVisualDirection('[CRITICAL RETRY] The previous visualDirection was missing or under 200 chars. Emit visualDirection.markdown (200+ chars, sections 1-6) and tags now.');
    if (vdRetry.input) { vd = vdRetry.input; }
    // eslint-disable-next-line no-console
    console.warn('[runAnalyzeTokens] visualDirection retry markdownLen:', vd?.markdown?.length || 0);
  }
  ...
  if (p1Empties.length > 0 || p1Rationale.length > 0) {
    ...
    if (p1Empties.length > 0) {
      parts.push(`Missing/short: ${p1Empties.join(', ')}. tokens (color 4-6, typography 3-4, layout 2-4, gradient 1-3).`);
    }
